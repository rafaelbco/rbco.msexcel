...
def rows_and_columns_to_matrix(rows_and_columns):
    def get_all_in_range(dict):
        return [dict.get(i) for i in xrange(max(dict.keys()))]
    
    new_sheets = {}
    for (sheet, rows) in rows_and_columns.iteritems():
        rows = [get_all_in_range(cols) for cols in get_all_in_range(rows)]
        new_sheets[sheet] = rows    
